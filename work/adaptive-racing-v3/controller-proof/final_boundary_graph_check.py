...
for B in (2,48,2,48,1):
    ...
    with torch.no_grad():
        ...
        if B == 2:
            fault_speed = args_cpu[1].clone(); fault_speed[0] = float('nan')
            fault_wheel = args_cpu[1].clone(); fault_wheel[0] = float('nan')
            cg.update_feedback(fault_wheel); gg.update_feedback(fault_wheel.cuda())
            c_fault = cpu(args_cpu[0], fault_speed, args_cpu[2], args_cpu[3])
            g_fault = gpu(args[0], fault_speed.cuda(), args[2], args[3])
            assert torch.isfinite(g_fault).all()
            torch.testing.assert_close(c_fault, g_fault.cpu(), atol=4e-4, rtol=4e-4)
            assert gg.last_diagnostics['input_fault'].tolist() == [True, False]
            assert gg.last_diagnostics['command_infeasible'][0]
            for tr, controller, dev in [(cpu,cg,'cpu'),(gpu,gg,'cuda')]:
                tr.reset(torch.arange(2,device=dev))
                controller.update_feedback(torch.zeros(2,device=dev))
            stop_launch = torch.zeros(2,8);stop_launch[0,6:]=-1;stop_launch[1,6:]=.6
            c_pair=cpu(stop_launch,torch.zeros(2),torch.full((2,),10.),torch.zeros(2))
            g_pair=gpu(stop_launch.cuda(),torch.zeros(2,device='cuda'),torch.full((2,),10.,device='cuda'),torch.zeros(2,device='cuda'))
            torch.testing.assert_close(c_pair,g_pair.cpu(),atol=4e-4,rtol=4e-4)
            assert torch.equal(g_pair[0],torch.zeros(2,device='cuda'))
            assert g_pair[1,1]>0
            assert not gg.last_diagnostics['input_fault'].any()
            gg.update_feedback(args[1])
        ...
        gpu._solver(*args)
    result={'batch':B,'capture_seconds':capture,'acceleration_status':gg.acceleration_status,'max_cpu_gpu_errors':errors,'graph_solver_ms':solver_ms,
...
Path('work/adaptive-racing-v3/controller-proof/graph_results_final_boundary.json').write_text(json.dumps(results,indent=2))
