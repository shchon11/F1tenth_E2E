...
original=gc.project_local_control
compiled=torch.compile(original,fullgraph=True,dynamic=False,mode='default')
@torch.no_grad()
def fused(*a,**kw):return compiled(*a,**kw)
gc.project_local_control=fused
for B in (2,48):
 ...
 t=mpc.PlanTracker(B,'cuda',.3302,.4189,10.,sp,False);g=gc.GripMPC(t,gs,B,'cuda',.3302,.4189,10.)
