...
def test_partial_reset_keeps_terminal_privileged_and_refreshes_current_state(tracks, monkeypatch):
    ...
    torch.testing.assert_close(info["final_obs"]["speed"], terminal["odom"][:1, 3:4] / 8)
    r = env.last_result
    ...
    torch.testing.assert_close(r.odom[:, 3:4] / 8, obs["speed"])
...
def test_plan_tracker_receives_reset_speed_and_imu(tracks, monkeypatch):
    ...
    torch.testing.assert_close(measured["speed"], obs["speed"][:, 0] * 8)
...
def test_first_finish_crossing_starts_full_lap_timer(tracks, monkeypatch):
    # Given the first finish crossing at step 100 after a mid-track spawn.
    env = make_env(tracks)
    env.ecfg.max_steps = 1000
    original_step = env.sim.step
    lap = 1
    ...
    env.ep_step.fill_(199)
    lap = 2
    # When the next finish crossing occurs at step 200.
    *_, info = env.step(torch.zeros(2, 2))
    # Then the full lap excludes the initial partial lap.
    torch.testing.assert_close(info["lap_times"], torch.full((2,), 100 * env.sim.control_dt))
...
def test_reset_scan_includes_opponents(tracks, monkeypatch):
    ...
    # Then LiDAR receives the boxes of the opponent at its actual pose.
    assert scan_cars[0] is not None
    torch.testing.assert_close(scan_cars[0], env.sim._car_boxes(env.state))
...
def test_speed_cap_gate_keeps_history_across_logging(tracks, monkeypatch, tmp_path):
    ...
    def capture_conditioning(model, scan, proprio):
        conditioning.append((proprio[:, 5].clone(), env.speed_cap.clone() / 8))
        return original_sample(model, scan, proprio)
