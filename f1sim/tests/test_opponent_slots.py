...
def test_a_kind_this_tree_does_not_have_is_named_and_refused():
    """`interactive` is worker 17's teacher. Listed, disabled, and refused with the reason -- never
    silently mapped onto the raceline teacher, which would be a run whose label is a lie."""
    kind = osl.kind_of("interactive")
    assert not kind.available and "worker 17" in kind.unavailable_message()
    with pytest.raises(ValueError) as exc:
        osl.validate_slots(osl.parse_slots('[{"kind": "interactive"}]'), 2)
