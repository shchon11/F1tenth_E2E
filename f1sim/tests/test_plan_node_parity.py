...
def test_cpu_tracker_matches_env_tracker():
    ...
    wb = env.cfg.vehicle.lf + env.cfg.vehicle.lr
    node = PlanTracker(1, "cpu", wb, env.cfg.vehicle.s_max, env.ecfg.v_max_policy)
    fast = PlanTrackerFast(wb, env.cfg.vehicle.s_max, env.ecfg.v_max_policy)      # what the ROS node actually runs
    i = 1; worst = (0.0, 0.0); worst_f = (0.0, 0.0)
    for t in range(60):
        a = env.teacher_label(teacher)
        lr = env.last_result
        v_meas = lr.odom[i, 3].item(); yaw_rate = lr.imu[i, :, 2].mean().item(); delay = env.tracker_delay[i].item()
        cmd_node = node(a[i:i + 1].cpu(), torch.tensor([v_meas]), torch.tensor([5.0]), torch.tensor([yaw_rate]), delay=delay)[0]
        cmd_fast = fast(a[i].cpu().numpy(), v_meas, 5.0, yaw_rate, delay=delay)
        env.step(a)
        ...
        worst_f = (max(worst_f[0], abs(cmd_fast[0] - float(cmd_env[0]))), max(worst_f[1], abs(cmd_fast[1] - float(cmd_env[1]))))
    assert worst[0] < 2e-3 and worst[1] < 0.05, worst        # rad, m/s
    assert worst_f[0] < 2e-3 and worst_f[1] < 0.05, worst_f
