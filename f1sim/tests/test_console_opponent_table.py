...
def test_a_kind_this_tree_does_not_have_is_listed_disabled_with_its_reason(qapp, monkeypatch):
    """The rule, exercised with a kind whose module is genuinely missing.

    `interactive` was that kind while it lived on worker 17's branch; since 2026-09-16 it is here
    and selectable (the test below). What must keep working is the case the machinery was built
    for: listed so that "no such kind" and "not here yet" stay different sentences, disabled so it
    cannot be chosen, and carrying the reason.
    """
    branch = osl.DriverKind("branchteacher", "branch 티처", "a kind that lives on a branch",
                            teacher=True, module="f1sim.not_merged_yet",
                            teacher_factory="f1sim.not_merged_yet:BranchTeacher",
                            pending="아직 병합되지 않은 브랜치에 있습니다.")
    monkeypatch.setattr(osl, "KINDS", osl.KINDS + (branch,))
    monkeypatch.setitem(osl.KIND_BY_NAME, branch.name, branch)
    t = OT.OpponentSlotTable()
    try:
        t.set_count(1)
        combo = t._rows[0].kind
        i = combo.findData("branchteacher")
        assert i >= 0, "an unavailable kind was dropped from the list instead of being explained"
        assert not (combo.model().item(i).flags() & QtCore.Qt.ItemIsEnabled)
        assert "병합" in combo.itemData(i, QtCore.Qt.ToolTipRole)
...
def test_the_interactive_teacher_is_selectable_now_that_it_is_merged(table):
    """The user's question -- "비쥬얼라이저의 티쳐는 그 업그레이드된 티쳐로 동작하는거지?" -- as a test.

    It is a *selection*: `interactive` is offered and enabled, and `raceline` is still offered and
    still the default, so nothing was replaced. Picking it produces a slot the trainer's own
    validator accepts.
    """
    table.set_count(1)
    combo = table._rows[0].kind
    i = combo.findData("interactive")
    assert i >= 0 and (combo.model().item(i).flags() & QtCore.Qt.ItemIsEnabled)
    assert "병합 후 활성" not in combo.itemText(i)
    assert combo.findData("raceline") >= 0
    combo.setCurrentIndex(i)
    slots = table.slots()
    assert [s.kind for s in slots] == ["interactive"]
    osl.validate_slots(slots, 2, require_files=False)
    assert table.problem(2) == ""
