...
That matters right now: worker 17's `InteractiveTeacher` is not on main, so `interactive` is listed
with `available = False` and a message saying so. A UI that offers it greys it out and says why; a
flag that names it is refused with the same sentence. What it is *not* is silently mapped onto the
raceline teacher -- a run that says `interactive` and drives a raceline is a result nobody can read.
