...
The same flag PPO takes, and the same block: nearest two cars, appended after every column the
observation already had, presence-gated, zero-initialised on a warm start.
**["Privileged opponent tokens" below](#privileged-opponent-tokens---opp-token--an-oracle-never-a-policy)
is the reference** — the layout, the four modes, the measured accuracy of the `future` columns and
the six refusals are all there, and `f1sim/opp_token.py` is the code. Nothing about it is different
for DAgger.

Two things belong here rather than there:

* `--opp-token` and `--teacher interactive` are **independent**. The block is what the *student*
  sees; the interactive teacher reads the other cars through `car_future` whatever the student's
  observation is. Running both is the "whole chain" command below; running either alone is a
  perfectly good arm.
* `--opp-future-model` is the **interactive teacher's** prediction, not the block's
  (`pred` / `hybrid` / `plan` / `constv`, measured in
  [interactive-teacher-2026-09-15](research/interactive-teacher-2026-09-15.md)). The block's future
  columns are fixed at `opp_token.OPP_TOKEN_PLAN_SOURCE`.

How well either prediction tracks the realised future is measured, not assumed:
